...
def main(def_args=sys.argv[1:]):
    ...
    for day in (start_date + timedelta(n) for n in range(NUM)):
        if (not no_weekends or day.weekday() < 5) \
            ...
            for commit_time in (day + timedelta(hours=5+m, minutes=randint(0, 60), seconds=randint(0, 60))
                                for m in range(contributions_per_day(args))):
                # print(timedelta(hours=5+m, minutes=randint(0, 60), seconds=randint(0, 60)))
                # print(commit_time)
